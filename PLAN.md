...
| 2026-07-16 | **F15**: Pesan agent kembali ke **metode ccbot** — `toTelegramMarkdown()` (telegramify-markdown + `convertMarkdownTables`) → MarkdownV2, split-then-convert, fallback `escapeMarkdown` (`d00a1d6`). Status bubble & output bash `*`/```` ``` ```` → `<b>`/`<pre>` (`9d672da`). Memperbaiki balasan tampil mentah `## ...`. **TDD 7/7 PASS** (`f15`). Formatting kini **hybrid**: konten agent = MarkdownV2, UI bot = HTML. |
| 2026-07-17 | **F16**: Notif selesai kini nge-**mention** akun yang benar — `buildUserMention` `<a href="tg://user?id=…">` di handler text/foto/voice (`8bf04b7`). **TDD 3/3 PASS** (`f16`). |
| 2026-07-17 | **Bug P**: Relay reliability — pesan hilang (idle prematur + pre-mark) & dobel (dedup ephemeral) → **dedup persisten per-sesi** (`relayedParts: Map<sessionId,Set<partId>>`) + seed-first-discovery + `isSessionStillGenerating` guard (`65c750f`). **TDD 6/6 PASS** (`fp`). |
| 2026-07-17 | **Audit-remediation** (2 commit): `5e4529d` (audit-A) — HealthMonitor reset `restartAttempts` saat recovery (fix SPOF), event-processor tidak mati permanen setelah 10 error (backoff + resume), fix HTML-formatting leftover di permission/question/dirBrowser, real idle re-entrancy guard. `210015c` (audit-B) — **hapus total kode SSE** (`subscribeEvents`, handler `handleSession*`, tipe `OpenCodeEvent`/`EventHandler`), drop dep `undici` (sumber HIGH npm vuln) + `pkg`, hapus `tests/unit/` yang broken. `npm audit` kini 1 low (dev-only esbuild via tsx). Detail: `docs/13-audit-2026-07-17.md`. |
